class ReportParser:
    ...
    def _get_field_types(self):
        columns = self.data["reportMetadata"]["detailColumns"]
        column_details = self.data["reportExtendedMetadata"]["detailColumnInfo"]
        return {key: column_details[value]["dataType"] for key, value in enumerate(columns)}
    ...
    def kvstore_dict(self):
        ...
        for group in fact_map.values():
            rows = group["rows"]
            group_records = (row["dataCells"] for row in rows)

            for record in group_records:
                labelled_record = {}
                for key, value in enumerate(record):
                    if field_types[key] in ["date","int","percent"]:
                        labelled_record[field_labels[key]] = value["value"]
                    elif field_types[key] == "currency" and value["label"] != "-":
                        labelled_record[field_labels[key]] = value["value"]["amount"]
                    else:
                        labelled_record[field_labels[key]] = value["label"]
                records.append(labelled_record)
