...
class Connection:
    ...
    def __init__(self, username=None, password=None, security_token=None, sandbox=False, api_version='v29.0', proxies=None):
    ...
    def login(self, username, password, security_token):
        ...
        response = requests.post(url, request_body, headers=request_headers, proxies=self.proxies)
    ...
    def _get_metadata(self, url):
        return requests.get(url + '/describe', headers=self.headers, proxies=self.proxies).json()
    ...
    def _get_report_filtered(self, url, filters):
        ...
        return requests.post(url, headers=self.headers, json=metadata, proxies=self.proxies).json()

    def _get_report_all(self, url):
        return requests.post(url, headers=self.headers, proxies=self.proxies).json()
